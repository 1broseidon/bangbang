...
Options:
    ...
    -u, --uninstall        Uninstall bangbang
...
Examples:
    ...
    $0 --uninstall        # Uninstall bangbang
EOF
    exit 0
}
...
get_latest_release_url() {
    ...
    echo "https://github.com/${OWNER}/${REPO}/releases/latest/download/${BINARY_NAME}_${os_name}_${arch}.tar.gz"
}
...
# Function to uninstall bangbang
uninstall_bangbang() {
    write_status "Uninstalling bangbang..."

    if [ -f "$INSTALL_DIR/$BINARY_NAME" ]; then
        rm -f "$INSTALL_DIR/$BINARY_NAME"
    fi

    write_status "Bangbang uninstalled."
    exit 0
}

# Main installation function
install_bangbang() {
    write_status "Installing bangbang..."
    write_status "OS: $OS"
    ...
    if ! curl $CURL_OPTS "$RELEASE_URL" -o bangbang.tar.gz; then
        echo "Error: Failed to download release. Try --insecure if using a proxy." >&2
        exit 1
    fi

    if [ "$SKIP_VERIFY" = false ]; then
        verify_checksum "bangbang.tar.gz" "$RELEASE_URL"
    else
        write_status "Skipping checksum verification (--no-verify)"
    fi

    write_status "Extracting archive..."
    if ! tar xzf bangbang.tar.gz; then
    ...
    if command -v "$BINARY_NAME" >/dev/null 2>&1; then
        write_status "Installation verified"
    else
    ...
    echo "Run 'bangbang' after restarting your terminal."
